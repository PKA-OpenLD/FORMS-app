...
export default function AdminPanel({
  ...
  const toggleRule = (id: string) => {
    fetch(`/api/sensor-rules?id=${id}`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        enabled: !sensorRules.find((r) => r.id === id)?.enabled,
      }),
    }).then(() => {
      setSensorRules((prev) =>
  ...
  const deleteSensor = (id: string) => {
    fetch(`/api/sensors?id=${id}`, { method: "DELETE" }).then(() =>
      setSensors((prev) => prev.filter((s) => s.id !== id)),
    );
  };

  const deleteRule = (id: string) => {
    fetch(`/api/sensor-rules?id=${id}`, { method: "DELETE" }).then(() =>
      setSensorRules((prev) => prev.filter((r) => r.id !== id)),
    );
  };
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600">
                        Tuyến Đường Nguy Cơ Lũ
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 bg-red-500 rounded-full opacity-50"></div>
                      <span className="text-gray-600">
                        Khu Vực Nguy Cơ Tắc Đường
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-1 bg-red-500"></div>
                      <span className="text-gray-600">
                        Tuyến Đường Nguy Cơ Tắc Đường
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="mb-6 border-2 border-green-200 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            setNewSensor({ ...newSensor, actionType: "flood" })
                          }
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setNewSensor({ ...newSensor, actionType: "outage" })
                          }
                          className={`p-3 rounded-lg font-medium transition-all ${
                    ...
                    >
                      {isAddingSensor
                        ? "📍 Nhấp Vào Bản Đồ Để Đặt Cảm Biến..."
                        : "🗺️ Đặt Cảm Biến Trên Bản Đồ"}
                    </button>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                    {sensors.map((sensor) => (
                      <div
                        key={sensor.id}
                        className="bg-white border-2 border-gray-200 p-4 rounded-lg hover:border-green-300 transition-all hover:shadow-md group"
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="ml-7 space-y-1">
                              ...
                              <p className="text-sm text-gray-600">
                                <span className="font-semibold">Ngưỡng:</span>{" "}
                                {sensor.threshold}
                              </p>
                              <p className="text-xs text-gray-500 font-mono">
                                {sensor.location[1].toFixed(5)},{" "}
                                {sensor.location[0].toFixed(5)}
                              </p>
                              <span
                    ...
                    {sensors.length === 0 && (
                      <div className="text-center py-8 text-gray-400">
                        ...
                        <p className="text-sm">
                          Chưa có cảm biến nào được triển khai
                        </p>
                        <p className="text-xs mt-1">
                          Thêm cảm biến đầu tiên của bạn ở trên
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="space-y-4 bg-gradient-to-br from-purple-50 to-indigo-50 p-5 rounded-xl border-2 border-purple-200">
                        ...
                        <>
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  setNewRule({ ...newRule, operator: "AND" })
                                }
                                className={`p-3 rounded-lg font-medium transition-all ${
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  setNewRule({ ...newRule, operator: "OR" })
                                }
                                className={`p-3 rounded-lg font-medium transition-all ${
                                  newRule.operator === "OR"
                                    ? "bg-indigo-500 text-white shadow-md"
                          ...
                          <div>
                            ...
                            >
                              <option value="">-- Chọn cảm biến --</option>
                              {sensors
                                .filter((s) => s.id !== newRule.sensors[0])
                                .map((s) => (
                                  <option key={s.id} value={s.id}>
                                    {s.type === "water_level"
                                      ? "💧"
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          <button
                            type="button"
                            onClick={() =>
                              setNewRule({ ...newRule, actionType: "flood" })
                            }
                            className={`p-3 rounded-lg font-medium transition-all ${
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setNewRule({ ...newRule, actionType: "outage" })
                            }
                            className={`p-3 rounded-lg font-medium transition-all ${
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              setNewRule({ ...newRule, actionShape: "circle" })
                            }
                            className={`p-3 rounded-lg font-medium transition-all ${
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setNewRule({ ...newRule, actionShape: "line" })
                            }
                            className={`p-3 rounded-lg font-medium transition-all ${
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          ...
                          disabled={
                            !newRule.name ||
                            newRule.sensors.length === 0 ||
                            (newRule.type === "2-sensor" &&
                              newRule.sensors.length < 2)
                          }
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                      {sensorRules.map((rule) => {
                        const sensorNames = rule.sensors
                          .map(
                            (sid) =>
                              sensors.find((s) => s.id === sid)?.name ||
                              "Unknown",
                          )
                          .join(` ${rule.operator || ""} `);
                        return (
                          <div
                            key={rule.id}
                            className={`bg-white border-2 p-4 rounded-lg transition-all hover:shadow-md group ${
                              rule.enabled
                                ? "border-green-300"
                                : "border-gray-200"
                            }`}
                            ...
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">
                                  ...
                                  Vẽ{" "}
                                  {rule.actionShape === "circle"
                                    ? "hình tròn"
                                    : "đường thẳng"}{" "}
                                  cho{" "}
                                  <span
                                    className={
                                      rule.actionType === "flood"
                                        ? "text-blue-600"
                                        : "text-red-600"
                                    }
                                  >
              ...
              onSaveWorkflow={(nodes, edges) => {
                ...
                actionNodes.forEach((actionNode) => {
                  ...
                  if (logicNode) {
                    ...
                    const logicIncoming = edges.filter(
                      (e) => e.target === logicNode.id,
                    );
                    ...
                    if (sensorIds.length === 2) {
                      newRules.push({
                        ...
                        actionType: actionNode.data.actionType as
                          | "flood"
                          | "outage",
                        actionShape: actionNode.data.actionShape as
                          | "circle"
                          | "line",
                        enabled: true,
                      });
                    ...
                    // 1-sensor rule(s)
                    const sensorNodes = sourceNodes.filter(
                      (n) => n.type === "sensor",
                    );
                ...
                alert(
                  `✓ Đã tạo ${newRules.length} quy tắc tự động từ quy trình!`,
                );
