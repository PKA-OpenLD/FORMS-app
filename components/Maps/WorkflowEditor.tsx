...
export default function WorkflowEditor({ sensors, map, onSaveWorkflow, onSensorCreated }: WorkflowEditorProps) {
    ...
    // Edit node handler
    const handleEditNode = useCallback((id: string, currentData: any) => {
    ...
    const [tempSensorLocation, setTempSensorLocation] = useState<[number, number] | null>(null);
    ...
    useEffect(() => {
        ...
        const handleSensorPlacement = (e: any) => {
            ...
            if (typeof window !== 'undefined' && (window as any).maplibregl) {
                const marker = new (window as any).maplibregl.Marker({ color: '#10b981' })
                    .setLngLat([lng, lat])
                    .addTo(map);
    ...
    useEffect(() => {
        ...
        const handleMapClick = (e: any) => {
            ...
            // Add marker to map
            if (typeof window !== 'undefined' && (window as any).maplibregl) {
                const marker = new (window as any).maplibregl.Marker({ color: node.data.actionType === 'flood' ? '#3b82f6' : '#ef4444' })
                    .setLngLat([lng, lat])
                    .addTo(map);
            ...
            if (currentPoints.length === 0) {
                ...
                // Second point - done
                updateNodeData(selectingPointsFor, { points: [...currentPoints, [lng, lat]] });
        ...
        <div className="h-full flex flex-col">
            ...
            {showSensorList && (
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 border-b-2 border-green-200 p-4 max-h-64 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {sensors.map(sensor => (
                            <div key={sensor.id} className="bg-white border-2 border-green-200 rounded-lg p-3 text-sm hover:border-green-400 transition-all">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="text-lg">
                                        {sensor.type === 'water_level' ? '💧' : 
                                         sensor.type === 'temperature' ? '🌡️' : '💨'}
                                    </span>
                                    <p className="font-bold text-gray-800 text-xs">{sensor.name}</p>
                                </div>
            ...
            <div className="bg-gray-800 p-3 flex items-center gap-2 flex-wrap border-b-2 border-gray-700">
                ...
                <button
                    ...
                    title={sensors.length === 0 ? 'Thêm cảm biến trước' : 'Thêm kích hoạt dựa trên điều kiện'}
                >
                    ⚡ Kích Hoạt
                </button>
            ...
            {selectedNodeType && (
                <div className="bg-yellow-100 border-b-2 border-yellow-400 px-4 py-2 text-sm text-yellow-800 font-medium">
                    ✨ Nhấp bất kỳ đâu trên khung vẽ để đặt <span className="font-bold">{selectedNodeType.toUpperCase()}</span>
                </div>
            )}
            ...
            {creatingNewSensor && !showSensorForm && sensorLinePoints.length === 0 && (
            ...
            {creatingNewSensor && !showSensorForm && sensorLinePoints.length === 1 && (
                <div className="bg-green-100 border-b-2 border-green-400 px-4 py-2 text-sm text-green-800 font-medium">
                    📍 Tiếp tục nhấp để thêm điểm | Nhấn <strong>Enter</strong> khi xong (tối thiểu 2 điểm) | <strong>ESC</strong> để hủy
                </div>
            )}
            {creatingNewSensor && !showSensorForm && sensorLinePoints.length >= 2 && (
                <div className="bg-blue-100 border-b-2 border-blue-400 px-4 py-2 text-sm text-blue-800 font-medium">
                    ✓ Đã thêm {sensorLinePoints.length} điểm | Nhấp để thêm thêm | Nhấn <strong>Enter</strong> để hoàn tất | <strong>ESC</strong> để hủy
                </div>
            )}
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
                            <h3 className="text-xl font-bold mb-4 text-gray-800">
                                Edit {editingNode.type === 'logic' ? 'Logic' : editingNode.type === 'trigger' ? 'Trigger' : 'Action'} Node
                            </h3>
                            ...
                            {editingNode.type === 'logic' && (
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                onClick={() => updateNodeData(editingNode.id, { operator: 'AND' })}
                                            ...
                                            <button
                                                onClick={() => updateNodeData(editingNode.id, { operator: 'OR' })}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                            ...
                            {editingNode.type === 'action' && (
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, actionType: 'flood' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                                            ...
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, actionType: 'outage' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, actionShape: 'circle' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                                            ...
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, actionShape: 'line' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                            ...
                            {editingNode.type === 'trigger' && (
                                <div className="space-y-4">
                                        ...
                                        >
                                            {sensors.map(sensor => (
                                                <option key={sensor.id} value={sensor.id}>
                                                    {sensor.type === 'water_level' ? '💧' : sensor.type === 'temperature' ? '🌡️' : '💨'} {sensor.name}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, condition: 'active' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                                            ...
                                            <button
                                                onClick={() => setEditingNode({ ...editingNode, data: { ...editingNode.data, condition: 'inactive' }})}
                                                className={`p-3 rounded-lg font-medium transition-all ${
                                    ...
                                    {editingNode.data.actionShape === 'line' && (
                                        <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
                                            ...
                                            <div className="text-sm text-gray-600 mb-3">
                                                {editingNode.data.points?.length === 0 && 'Chưa chọn điểm nào'}
                                                {editingNode.data.points?.length === 1 && 'Đã chọn 1 điểm - nhấp vào bản đồ cho điểm thứ 2'}
                                                {editingNode.data.points?.length === 2 && '✓ Đã cấu hình đường'}
                                            </div>
                                            <button
                                                onClick={() => {
                                            ...
                                            >
                                                {editingNode.data.points?.length > 0 ? '🔄 Chọn Lại Điểm Trên Bản Đồ' : '🗺️ Chọn 2 Điểm Trên Bản Đồ'}
                                            </button>
                                            {editingNode.data.points?.length > 0 && (
                                                <button
                                                    onClick={() => {
                                                        setEditingNode({ ...editingNode, data: { ...editingNode.data, points: [] }});
                                                        tempMarkers.forEach(m => m.remove());
                                                        setTempMarkers([]);
                                                    }}
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
                            ...
                            <div className="bg-green-50 border-2 border-green-200 rounded-lg p-3 mb-4 text-sm text-green-800">
                                📍 Vị Trí Cảm Biến: {tempSensorLocation[1].toFixed(5)}, {tempSensorLocation[0].toFixed(5)}
                                <br />
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setNewSensorForm({ ...newSensorForm, type: e.target.value as any })}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setNewSensorForm({ ...newSensorForm, threshold: parseFloat(e.target.value) || 0 })}
                                            className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-green-400 focus:outline-none"
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            type="button"
                                            onClick={() => setNewSensorForm({ ...newSensorForm, actionType: 'flood' })}
                                            className={`p-3 rounded-lg font-medium transition-all ${
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setNewSensorForm({ ...newSensorForm, actionType: 'outage' })}
                                            className={`p-3 rounded-lg font-medium transition-all ${
            ...
            <div className="bg-gray-800 p-3 text-white text-xs space-y-1">
                ...
                <div>• <strong>Kích hoạt:</strong> Tự động hóa dựa trên điều kiện (cảm biến → hành động)</div>
                <div>• <strong>Logic:</strong> Kết hợp nhiều cảm biến với VÀ/HOẶC</div>
                <div>• <strong>Hành động:</strong> Xác định thức vẽ khi kích hoạt</div>
                <div>• <strong>Nhấp đúp</strong> vào bất kỳ Logic/Hành Động/Kích Hoạt để chỉnh sửa hoặc xóa</div>
                <div>• Kéo các ô để di chuyển • Kết nối các ô bằng cách kéo tay cầm</div>
            </div>
        </div>
    );
}
